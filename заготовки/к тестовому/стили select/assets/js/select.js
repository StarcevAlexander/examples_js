...
function select_actions(original, select) {
  ...
  select_item.addEventListener('mouseenter', function () {
    ...
    select.style.border = '1px solid #659DBD';
  });
  select_item.addEventListener('mouseleave', function () {
    let selects = document.querySelectorAll('.select');
    for (let index = 0; index < selects.length; index++) {
      const select = selects[index];
      const select_body_options = select.querySelector('.select__options');
    ...
    select.style.border = '1px solid #eaeaea';
  });
  for (let index = 0; index < select_options.length; index++) {
    ...
    select_option.addEventListener('click', function () {
      ...
      if (select_type == 'input') {
        ...
        select.querySelector('.select__value').innerHTML = `<span style="color: #232323">${select_option_text}</span>`;
        original.value = select_option_value;
        select_option.classList.add('_active');
        img1.style.display = "none"
        img2.style.display = "none"
        img3.style.display = "block"
        select.querySelector('.select__title').style.outline = '1px solid #A2E0B2';
        select.querySelector('.select__title').style.boxShadow = 'none';
      }
    });
  }
}
